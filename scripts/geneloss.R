...
hemiz_blast_partial<-tx_yloss %>% 
  filter(!tx_mat%in%tophit_notloss$qseqid &
          tx_mat%in%tophit_partial$qseqid) %>% 
  mutate(txtype = "partial")
...
tx_yloss_nc_overlap<-hemiz_blast_all %>% full_join(nc_all_loss, by = c("nc_hap1" = "repGene")) %>% 
  mutate(nctype = replace_na(nctype,"no overlap")) %>%
  mutate(txtype = replace_na(txtype,"no overlap")) %>%
  filter(txtype != "no overlap")

geneloss_summary<- tx_yloss_nc_overlap %>% group_by(txtype,nctype) %>% summarise(n = n())
