...
jpop_reads_filter<-  jpop_reads %>%rowwise() %>% mutate(total = sum(TM4:TM3)) %>% filter(.,total >20)
...
#pgMat_tx_wide<-fread("pangenes_tx/tx_mat_pg_synorths.txt")
#I don't like it!

# hemizygousGenes <-filter(pgMat_tx,(flag == "PASS") & (genome == "tx_mat"|genome == "tx_pat")) %>%
#    select(pgID,genome,id,chr,start, end, flag) %>% 
#    group_by(pgID,genome,flag) %>% 
#    filter(n() == 1) %>% 
#    ungroup() %>%
...
#    mutate(across(.cols = c("id_tx_pat","id_tx_mat"), 
#                  ~str_replace(.,"-RA$",""))) %>%
#    filter(is.na(id_tx_pat)) %>%
#    distinct()

### outgroup - paucifolius
##. or use NC set
## vetted with blast searches


hemizygousGenes <-filter(pgMat_tx,(flag == "PASS") & 
                           (genome == "tx_mat"|genome == "tx_pat"|genome =="Rsal")) %>%
   select(pgID,genome,id,chr,start, end, flag) %>% 
   group_by(pgID,genome,flag) %>% 
   filter(n() == 1) #%>% 
   ungroup() %>%
   ...
   mutate(across(.cols = c("id_tx_pat","id_tx_mat"), 
                 ~str_replace(.,"-RA$",""))) %>%
   filter(is.na(id_tx_pat)) %>% filter(!is.na(id_Rsal)) %>%
   distinct()
...
hemicounts <- as.data.frame(normalized_counts_auto) %>% 
  rownames_to_column(var = "Geneid") %>% filter(Geneid%in%hemizygousGenes$id_tx_mat) %>%
  rowwise %>% 
...
#ggsave("hemizygous_jpop_RsalOutgroup.pdf", height = 5, width =7)
```
...
- 
```{r}
gametologCounts<-as.data.frame(normalized_counts_auto) %>% 
  rownames_to_column(var = "Geneid") %>% 
  filter(!Geneid%in%genesToRemove$id_tx_mat) %>%
  filter(!Geneid%in%genesToRemove$id_tx_pat) %>%
  rowwise %>% 
  mutate(maleMean = mean(c_across(contains("TM")))) %>%
  mutate(femaleMean = mean(c_across(contains("TF"))))
```
