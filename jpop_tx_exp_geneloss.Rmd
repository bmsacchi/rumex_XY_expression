...
nohit<- anti_join(genelist_tx_mat,hits, by = c("tx_mat"="qseqid")) ## 222 with no hit
...
heads<-c("genespace_yloss","blast_confirmed_loss","complete_loss","partial_loss")
ylo<-(filter(summary_loss,chr =="X"))$n_loss

vals<-c(376,352,222,130)
...
overlap_all_loss<-tx_mat_loss %>% filter(nc_hap1%in%allncGenesToKeep$repGene)
# nvm basically the same, just one diff

#overlap with blast loss:
